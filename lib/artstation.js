...
let sendTop10 = channel => {
  let mixSend = (message, topArt) => {
    ...
    message.channel.send(chosen).then( res => log.logSend(res)).catch(e => log.logError(e))
    sended.push(chosen)
...
let send = (message, topArt) => {
  ...
  if (top) {
    ...
    message.channel.send(pandemonium.choice(topArt).permalink).then( res => log.logSend(res)).catch(e => log.logError(e))
  }
}
