...
let check = (message, db) => {
  ...
  if (msg[0] === "add") {
    ...
    let userId = message.author.id
    if (!/\d+/.test(msg)) {
      message.channel.send("Укажите ID конкретного таска a не " + msg).then( res => log.logSend(res))
      return
    }
    ...
    if (delPost.length === 0) {
      message.channel.send("Не найдена таска с таким id: " + msg).then( res => log.logSend(res))
      return
    }
    if (delPost[0].uid !== userId) {
      console.log(delPost[0].uid, userId)
      message.channel.send("Вы не владелец этой таски с id: " + msg).then( res => log.logSend(res))
      return
    }
    db
      .get("todo")
      ...
      .write()
    message.channel.send("Удолил:\n" + "```" + delPost[0].text + "```").then( res => log.logSend(res))
  } else {
    let byUser = msg[0]
    let todo = []
    if (byUser !== undefined) {
