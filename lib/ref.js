...
function run(message, db) {
  ...
  if (_.toLower(msg[0]) === "add") {
      ...
      .then(function(response) {
        ...
        if (
          ...
          let warnMsg =
            ...
            " MB"
          message.channel.send(warnMsg).then( res => log.logSend(res))
          log.warn(warnMsg)
          return
        }
    ...
    if (tag === undefined || tag == "") {
      message.channel.send("Не вижу вашего тега.").then( res => log.logSend(res))
      return
    }
    ...
    if (newTags === undefined || newTags.length === 0) {
      message.channel.send("Текущие теги:\n" + found[0].tags.join(", ")).then( res => log.logSend(res))
      return
    }
    ...
    if (existedTags.length > 0) {
      message.channel.send("Эти теги уже существуют: " + existedTags.join(", ")).then( res => log.logSend(res))
    }
    ...
    let data = db
      ...
      .uniq()
      .value()
    message.channel.send("all tags:\n" + data.join(", ")).then( res => log.logSend(res))
    ...
    if (addTags.length === 0 && delTags.length === 0) {
      ...
      if (idData === undefined) {
        message.channel.send("Не нашла такой картинки с таким номером").then( res => log.logSend(res))
        return
      }
    ...
    let exTags = []
    tags = tags.filter(v => /^!.+/.test(v) ? exTags.push(_.trimStart(v, '!')) && false : true)
    ...
    let fd = db
      ...
      .filter(ex => !exTags.map(tag => search(ex.tags, [tag])).some(res => res === true))
      .sample()
      .value()
