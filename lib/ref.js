...
function run(message, db) {
  ...
  if (_.toLower(msg[0]) === "add") {
    ...
    let cleanedTags = addTags.reduce((holder, value) => {
      ...
      console.log("existed:", existTag)

      if (existTag.length === 0) holder.push(value)
      return holder
    }, [])

    console.log(cleanedTags)
    if (cleanedTags === undefined || cleanedTags.length === 0) {
      console.log("exit")
      return
    }
    ...
    //removeTags
    _.remove(tagsToAssign, x => {
      return _.includes(delTags, x)
    })
    //removeTags End
    ...
    let search = (tagsFromDb, curTags) => {
      ...
      console.log(allCorrect, curTags, tagsFromDb)
